...
func (h *Handler) handleMessage(ctx context.Context, conn *websocket.Conn, client *entity.Client, msg Message) error {
	...
	case "offer", "answer", "ice-candidate":
		...
		for _, c := range room.GetClients() {
			if c.ID != client.ID {
				// В реальном приложении здесь нужно реализовать отправку сообщения конкретному клиенту
				// Это потребует дополнительной логики для хранения активных соединений
			}
		}
